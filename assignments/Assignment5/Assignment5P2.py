...
def transform_img(path, save_path):
    """
    ...
    flipped_image = cv2.flip(corrected_img, 0)
    cv2.imwrite(save_path, flipped_image)
    plt.imshow(flipped_image)
    plt.title('Corrected Flipped IMG')
    plt.show()
...
def extract_features(img_path: str) -> dict:
    ...
    for color_name, color_value in colors:
        ...
        for contour in contours:
            ...
            if color_name == "turqoise" and roundness < 0.7:
                shape = "circle"
            print("current object: ", color_name, shape, area, roundness)
            objects.append(
                {
                    "color" : color_name,
                    "shape" : shape,
...
# 2d
def annotate_features(path2, path3, objects):
    # define colors
    colors = {
        "orange": (0, 37, 149),
    ...
    img = cv2.imread(path2) # read in image
...
def get_features():
    path1 = "test-image-4-7.png"
    path2 = "test-image-4-7-2.png"
    path3 = "test-image-4-7-3.png"
    img = get_img(path1)
    img = transform_img(path1, path2)
    objects = extract_features(path2)
    # return objects
    annotate_features(path2, path3, objects)
    return objects
